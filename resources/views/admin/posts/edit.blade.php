...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">{{ __('Edit Post') }}</h1>
        </div>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('admin.posts.update', $post) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="blog_id" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('Blog') }}
                    </label>
                    <select class="border border-gray-300 rounded-lg w-full px-4 py-2 focus:ring-blue-500 focus:border-blue-500" name="blog_id" id="blog_id" required>
                        <option value="" disabled>{{ __('Choose a Blog') }}</option>
                        @foreach($blogs as $blog)
                            <option value="{{ $blog->id }}" {{ old('blog_id', $post->blog_id) == $blog->id ? 'selected' : '' }}>{{ $blog->title }}</option>
                        @endforeach
                    </select>
                    @error('blog_id')
